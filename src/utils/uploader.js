...
const ERROR_TYPE_VERSION = 'VERSION_ERROR';
const ERROR_TYPE_OTHER = 'GENERAL_ERROR';
...
class Uploader {
    ...
    static async create(pk, rpc, chainId, contractAddress, uploadType) {
        ...
        return { success: false, errorType: uploader.#errorType, errorMessage: uploader.#errorMessage };
    }
    ...
    async #init(pk, rpc, chainId, contractAddress, uploadType) {
        ...
        if (uploadType) {
            if (!this.#flatDirectory.isSupportBlob && uploadType === UploadType.Blob) {
                this.#errorType = ERROR_TYPE_OTHER;
                this.#errorMessage = `ERROR: The current network does not support this upload type, please switch to another type. Type=${uploadType}`;
                return false;
            }
